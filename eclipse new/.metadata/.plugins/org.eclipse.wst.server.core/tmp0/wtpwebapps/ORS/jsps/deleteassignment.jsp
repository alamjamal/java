...
%>
<%
	ArrayList<CourseTO> ar=new CourseDAOImpl().getAssignmentList();
%>
	...
	<table width="100%" align="left" border="0" style="vertical-align: top;">
		...
		<tr>
			<td colspan="1"  valign="top">
						...
						<font color="#1B577C" size="4"><b><i>Welcome On Request Coordinater,Here You Can View Assignments..</i></b></font>
					<br/>
...
%>
	    ...
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">SheduleId</font></div></td>
	    ...
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">FacultyLoginId</font></div></td>
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">FacultyName</font></div></td>
         ...
         <tr bgcolor="#C3D7BA">
      	...
      	<td bgcolor="#FFFFFF" width="40"><center><%=pd.getSheduleid()%></center></td>
    	<td bgcolor="#FFFFFF" width="180"><center><%=pd.getCoursename()%> </center></td>
    	<td bgcolor="#FFFFFF" width="220"><center><%=pd.getCoursedetails()%> </center></td>
    	...
    	<td bgcolor="#FFFFFF" width="80"><center><%=pd.getFacultyloginid()%> </center></td>
    	<td bgcolor="#FFFFFF" width="80"><center><%=pd.getFacultyname()%> </center></td>
    	
      <%} %>
      </table>
      <table align="center"><tr><td width="100%">
      (<%=startrow %>-<%if(endrow>totalrows)endrow=totalrows;%><%=endrow %> of <%=totalrows %> Total)
      <%if(pageNo>0) {%><a href='<%=request.getContextPath()%>/jsps/deleteassignment.jsp'>First</a>|<a href='<%=request.getContextPath()%>/jsps/deleteassignment.jsp?page=<%=pageNo-1%>'><< |</a><%} %>
      <%
     // int count=0;
      for(int i=0;i<totalpages;i++){
      if((i==pageNo-2)||(i==pageNo-1)||(i==pageNo)||(i==pageNo+1)||(i==pageNo+2)){
      if(!(pageNo==i)){%><a href='<%=request.getContextPath()%>/jsps/deleteassignment.jsp?page=<%=i %>'><%}%> <%=(i+1) %></a> | 
      <%//count++;
      }
      //if(count==5)break;
      }
      %><%if(pageNo<totalpages-1) {%><a href='<%=request.getContextPath()%>/jsps/deleteassignment.jsp?page=<%=pageNo+1%>'>>> |</a><a href='<%=request.getContextPath()%>/jsps/deleteassignment.jsp?page=<%=totalpages-1%>'>Last</a><%} %></td></tr>
