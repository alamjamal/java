	...
	<table width="100%" align="left" border="0" style="vertical-align: top;">
		...
		<tr>
			<td colspan="1"  valign="top">
						...
						<font color="#1B577C" size="4"><b><i>Welcome On Request Coordinater,Here You Can View And Remove Old Sheduled Courses..</i></b></font>
					<br/>
	...
	String pa = request.getParameter("page");
         ...
         if(totalrows%5!=0)
  	     ...
  	     if(endrow>totalrows)
  	          endrow=totalrows;
%>
  	     <form action=<%=request.getContextPath()+"/DeleteSheduleAction" %> method="post" name="deleteform">
	    ...
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">ExpectedEndingDate</font></div></td>
          ...
          <%
	    	              ...
	    	              %>
         <tr bgcolor="#C3D7BA">
      	<td bgcolor="#FFFFFF"><input type="checkbox" name="crid" value="<%=pd.getSheduleid()%>"></td>
      ...
      <%if(pageNo>0) {%><a href='<%=request.getContextPath()%>/jsps/deleteshedule.jsp'>First</a>|<a href='<%=request.getContextPath()%>/jsps/deleteshedule.jsp?page=<%=pageNo-1%>'><< |</a><%} %>
      <%
     // int count=0;
      for(int i=0;i<totalpages;i++){
      if((i==pageNo-2)||(i==pageNo-1)||(i==pageNo)||(i==pageNo+1)||(i==pageNo+2)){
      if(!(pageNo==i)){%><a href='<%=request.getContextPath()%>/jsps/deleteshedule.jsp?page=<%=i %>'><%}%> <%=(i+1) %></a> | 
      <%//count++;
      }
      //if(count==5)break;
      }
      %><%if(pageNo<totalpages-1) {%><a href='<%=request.getContextPath()%>/jsps/deleteshedule.jsp?page=<%=pageNo+1%>'>>> |</a><a href='<%=request.getContextPath()%>/jsps/deleteshedule.jsp?page=<%=totalpages-1%>'>Last</a><%} %></td></tr>
     <tr><td colspan="5" align="right"><input type="submit" name="submit" value="Delete Selected Course Shedules" onclick="javascript:return get_check_value()"/></td></tr>
      <%
