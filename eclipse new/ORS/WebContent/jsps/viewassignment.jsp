...
ArrayList<CourseTO> ar=new CourseDAOImpl().getFacultyAssignmentList(username);
%>
	...
	<table width="100%" align="left" border="0" style="vertical-align: top;">
		...
		<tr>
			<td colspan="1"  valign="top">
						...
						<font color="#1B577C" size="4"><b><i>Welcome <%=username%>,Here You Can View Those Course Shedules Which Are Assigned To You.. </i></b></font>
					<br/>
...
%>
      ...
      <%if(pageNo>0) {%><a href='<%=request.getContextPath()%>/jsps/viewassignment.jsp'>First</a>|<a href='<%=request.getContextPath()%>/jsps/viewassignment.jsp?page=<%=pageNo-1%>'><< |</a><%} %>
      <%
     // int count=0;
      for(int i=0;i<totalpages;i++){
      if((i==pageNo-2)||(i==pageNo-1)||(i==pageNo)||(i==pageNo+1)||(i==pageNo+2)){
      if(!(pageNo==i)){%><a href='<%=request.getContextPath()%>/jsps/viewassignment.jsp?page=<%=i %>'><%}%> <%=(i+1) %></a> | 
      <%//count++;
      }
      //if(count==5)break;
      }
      %><%if(pageNo<totalpages-1) {%><a href='<%=request.getContextPath()%>/jsps/viewassignment.jsp?page=<%=pageNo+1%>'>>> |</a><a href='<%=request.getContextPath()%>/jsps/viewassignment.jsp?page=<%=totalpages-1%>'>Last</a><%} %></td></tr>
      <%
