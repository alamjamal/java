...
<html>
	...
	<body topmargin="0" leftmargin="0" marginwidth="0" marginheight="0"
		background=<%=request.getContextPath() + "/images/web-background.png"%>>
				...
				<td colspan="1" valign="top">
					<c:choose>
						<c:when test="${sessionScope.role eq 'department'}">
							...
							<font color="#1B577C" size="4"><b><i>Welcome H.O.D
										Of Department <%=new ProfileDAOImpl()
									.getDeptname(((String) session
											.getAttribute("user")))%>,Here
										You Can View The Course Shedule...</i>
							</b>
							</font>
						</c:when>
						<c:when test="${sessionScope.role eq 'faculty'}">
							<jsp:include page="usermenu.jsp"></jsp:include><br />
							<font color="#1B577C" size="4"><b><i>Welcome <%=(String) session.getAttribute("user")%>,Here
										You Can View The Course Shedule...</i>
							</b>
							</font>
						</c:when>
						...
						<tr bgcolor="#cyan">
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">Sl No</font>
								</div>
							</td>
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">CourseName</font>
								</div>
							</td>
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">CourseDetails</font>
								</div>
							</td>
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">StartingDate</font>
								</div>
							</td>
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">ExpectedEndingDate</font>
								</div>
							</td>
							<td bgcolor="#E0FEFD">
								<div align="center" class="style16">
									<font color="#1B577C">Request</font>
								</div>
							</td>
							...
							<tr bgcolor="#C3D7BA">
								...
								<td bgcolor="#FFFFFF" width="200">
									<center><%=pd.getCoursename()%>
									</center>
								</td>
								<td bgcolor="#FFFFFF" width="250">
									<center><%=pd.getCoursedetails()%>
									</center>
								</td>
								<td bgcolor="#FFFFFF" width="120">
									<center><%=pd.getCoursestartdate()%>
									</center>
								</td>
								<td bgcolor="#FFFFFF" width="120">
									<center><%=pd.getCourseenddate()%>
									</center>
								</td>
								...
								<td bgcolor="#FFFFFF" width="120">
									<center>
										<%=pd.getCoursename()%>
										<a
											href="<%=basePath%>EmpRequestAction?coursename=<%=pd.getCoursename()%>">Request</a>
					...
					<table align="center">
						<tr>
							...
							%><%=endrow%>
								...
								if (pageNo > 0) {
							%><a
									href='<%=request.getContextPath()%>/jsps/viewshedule.jsp'>First</a>|
								<a
									href='<%=request.getContextPath()%>/jsps/viewshedule.jsp?page=<%=pageNo - 1%>'><<
									|</a>
								<%
									}
								%>
									...
									// int count=0;
											for (int i = 0; i < totalpages; i++) {
												if ((i == pageNo - 2) || (i == pageNo - 1)
													...
													if (!(pageNo == i)) {
								%><a
									href='<%=request.getContextPath()%>/jsps/viewshedule.jsp?page=<%=i%>'>
									<%
										}
									%> <%=(i + 1)%></a> |
								<%
 ...
 %>
								...
								%><a
									href='<%=request.getContextPath()%>/jsps/viewshedule.jsp?page=<%=pageNo + 1%>'>>>
									|</a><a
									href='<%=request.getContextPath()%>/jsps/viewshedule.jsp?page=<%=totalpages - 1%>'>Last</a>
								<%
									}
								%>
							</td>
