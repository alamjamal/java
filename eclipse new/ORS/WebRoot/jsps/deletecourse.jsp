	...
	<table width="100%" align="left" border="0" style="vertical-align: top;">
		...
		<tr>
			<td colspan="1"  valign="top">
						...
						<font color="#1B577C" size="4"><b><i>Welcome On Request Coordinator,Here You Can View And Delete Courses</i></b></font>
					<br/>
	...
	String pa = request.getParameter("page");
         ...
         if(totalrows%5!=0)
  	     ...
  	     if(endrow>totalrows)
  	          endrow=totalrows;
%>
  	     <form action=<%=request.getContextPath()+"/DeleteCourseAction" %> method="post" name="deleteform">
	    ...
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">CourseId</font></div></td>
	    ...
	    <td bgcolor="#E0FEFD"><div align="center" class="style16"><font color="#1B577C">CourseDetails</font></div></td>
          ...
          <%
	              ...
	              %>
         <tr bgcolor="#C3D7BA">
      	<td bgcolor="#FFFFFF"><input type="checkbox" name="crid" value="<%=pd.getCourseid()%>"></td>
      	<td bgcolor="#FFFFFF" width="40"><center><%=intIndex+1 %></center></td>
      	<td bgcolor="#FFFFFF" width="40"><center><%=pd.getCourseid()%></center></td>
    	<td bgcolor="#FFFFFF" width="70"><center><%=pd.getCoursename()%> </center></td>
      ...
      <%if(pageNo>0) {%><a href='<%=request.getContextPath()%>/jsps/deletecourse.jsp'>First</a>|<a href='<%=request.getContextPath()%>/jsps/deletecourse.jsp?page=<%=pageNo-1%>'><< |</a><%} %>
      <%
     // int count=0;
      for(int i=0;i<totalpages;i++){
      if((i==pageNo-2)||(i==pageNo-1)||(i==pageNo)||(i==pageNo+1)||(i==pageNo+2)){
      if(!(pageNo==i)){%><a href='<%=request.getContextPath()%>/jsps/deletecourse.jsp?page=<%=i %>'><%}%> <%=(i+1) %></a> | 
      <%//count++;
      }
      //if(count==5)break;
      }
      %><%if(pageNo<totalpages-1) {%><a href='<%=request.getContextPath()%>/jsps/deletecourse.jsp?page=<%=pageNo+1%>'>>> |</a><a href='<%=request.getContextPath()%>/jsps/deletecourse.jsp?page=<%=totalpages-1%>'>Last</a><%} %></td></tr>
      <tr><td colspan="5" align="right"><input type="submit" name="submit" value="Delete Selected Courses" onclick="javascript:return get_check_value()"/></td></tr>
      <%
