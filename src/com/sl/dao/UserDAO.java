...
import com.sl.google.GooglePojo;
...
public class UserDAO {
	...
	public static int incrementVerificationAttempts(String user_id) throws DBException{
		...
		try {
			...
			ps2 = conn.prepareStatement("SELECT EMAIL_VERIFICATION_ATTEMPTS from DEMO_USER where USER_ID = ?");
	...
	public static void profileUpdate(String fname, String lname, String userId) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = DBConn.getConnection();
			ps = conn.prepareStatement("update DEMO_USER set FIRST_NAME = ?, LAST_NAME=? where USER_ID = ?");
	...
	public static int profileUpdate(UserPojo up) throws DBException{
		int i=0;
		Connection conn = null;
		PreparedStatement ps = null;
		...
		try {
			conn = DBConn.getConnection();
			ps = conn.prepareStatement("update DEMO_USER set FIRST_NAME = ?, LAST_NAME=? where EMAIL = ?");
			ps.setString(1,up.getFIRST_NAME());
			ps.setString(2,up.getLAST_NAME());
	...
	public static int googleRegister(GooglePojo gp) throws DBException {
		int i=0;
		Connection conn = null;
		PreparedStatement ps = null;
		System.out.println("i am in dao class hurraa");
		try {
			conn = DBConn.getConnection();
			ps = conn.prepareStatement("insert into google_user (id,name,email) values(?,?,?)");
			ps.setString(1,gp.getId());
			ps.setString(2,gp.getName());
			ps.setString(3,gp.getEmail());
			int j = ps.executeUpdate();
			if(j!=0){
	...
	public static int googleCheckEmail(GooglePojo gp) throws DBException{
		int j=0;
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try{
			conn = DBConn.getConnection();
			ps= conn.prepareStatement("select 1 from google_user where email=?");
			ps.setString(1, gp.getEmail());
			rs = ps.executeQuery();
			while(rs.next()){
				j++;
			}
